...
window.Debugger = (function() {
  ...
  function jsonToHtml(jsonObj) {
          ...
          if (descriptionValue !== '') {
            html += '正文内容：<div style="border: 1px solid #ccc; padding: 10px;">' + descriptionValue + '</div>';
          }
          return html;
        };
  ...
  return {
    ...
    run: function() {
      ...
      $.ajax({
        type: "POST",
        url: location.pathname+'/pyspider/run',
        data: {
...
//              $("#tab-web iframe").height(doc.body.scrollHeight + 60);
                var iframeHeight = doc.body.scrollHeight === 0 ? 1000 : doc.body.scrollHeight + 60;
                $("#tab-messages iframe").height(iframeHeight);
            }
          };
          ...
          if (data.messages.length > 0) {
            ...
            // console.log("ddddd111",messages)
            // CodeMirror.runMode(messages, 'application/json', $('#tab-messages pre')[0]);
            // $('#tab-messages')[0]
            // console.log("ddddd22222222",messages)
            var messageContent = jsonToHtml(data.messages[0][1]);
            // console.log("dypsss",messageContent)
            // 添加一个 iframe 容器
            $('#tab-messages .iframe-box').html('<iframe sandbox="allow-same-origin allow-scripts" height="50%"></iframe>');
            var messageIframe = $("#tab-messages iframe")[0];
            // 确保 iframe 宽高与父元素相同
            $(messageIframe).css({
              width: '100%',
            //  height: '100%',
            ...
            // 在 iframe 中显示渲染后的 HTML
              const doc = messageIframe.contentDocument;
              doc.open("text/html", "replace");
              doc.write(messageContent);
              doc.close();
              doc.onreadystatechange = () => {
                if (doc.readyState === 'complete') {
                  const iframeHeight = doc.body.scrollHeight === 0 ? 1000 : doc.body.scrollHeight + 60;
                  $("#tab-messages iframe").height(iframeHeight);
                }
              };
                  // 当内容加载完成后，调整 iframe 的高度
                $(messageIframe).on('load', function() {
                  const iframeHeight = doc.body.scrollHeight === 0 ? 1000 : doc.body.scrollHeight + 60;
                  $(messageIframe).height(iframeHeight);
                });
