...
def connect_message_queue(name, url=None, maxsize=0, lazy_limit=True):
    ...
    if parsed.scheme == 'amqp':
        ...
        if ',' in parsed.netloc:
            ...
            #print("message_queue---redis_queue", "name=", name, "host=", host, "port=", port, "db=", db, "maxsize=", maxsize, "lazy_limit=", lazy_limit, "password=", password, "username=", username)
            redis_connect = Queue(name=name, host=host, port=port, db=db, maxsize=maxsize,
                         lazy_limit=lazy_limit, password=password, username=username)
