...
class BaseHandler(object):
    ...
    def _run_func(self, function, *arguments):
        ...
        if process_time_limit > 0:
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(function, *arguments[:len(args) - 1])
                try:
                    ret = future.result(timeout=process_time_limit)
                except TimeoutError:
                    err_info = f'process time limit:{process_time_limit}, process timeout{time.time() - start_time}'
                    self.logger.exception(err_info)
                    raise TimeoutError()
        else:
            ret = function(*arguments[:len(args) - 1])
        # self.logger.info(f"Time spent processing function bodies {time.time()-start_time}, process time limit:{process_time_limit}")
        return ret
    ...
    fetch_fields = ('method','params', 'headers', 'user_agent', 'data', 'connect_timeout', 'timeout', 'allow_redirects', 'cookies',
    ...
    def get_taskid(self, task):
        ...
        return md5string(task['url'] + json.dumps(task['fetch'].get('data', '')) + json.dumps(task['fetch'].get('params', '')))
    ...
    def css_to_xpath(self, css_selector):
        try:
            xpath = GenericTranslator().css_to_xpath(css_selector)
            return xpath
        except SelectorError as e:
            s = f"Error converting CSS to XPath: {e}"
            print(s)
            return None
