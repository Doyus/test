...
def _connect_database(url):  # NOQA
    ...
    if engine == 'mysql':
        ...
        if dbtype == 'taskdb':
            ...
            # password = parsed.netloc.split('@')[0][1:]
            # return TaskDB(parsed.hostname, parsed.port,
            #               int(parsed.path.strip('/') or 0),
            #               password=password)

            user_pass = parsed.netloc.split('@')
            if len(user_pass) == 2:  # 有用户名和密码
                user_info, host_port = user_pass
                username, password = user_info.split(':')
            else:  # 只有密码
                password = user_pass[0]
                username = None
                host_port = user_pass[1]

            host, port_str = host_port.split(':')
            port = int(port_str)
            db = int(parsed.path.strip('/') or 0)
            # print("taskdb---TaskDB", "name=", username, "host=", host, "port=", port, "db=", db, "password=", password, "username=", username)
            return TaskDB(host, port, db, username=username, password=password)
        else:
