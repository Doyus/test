...
def read_config(ctx, param, value):
    ...
    # # 初始化 ctx.obj 为空 ObjectDict
    # ctx.obj = utils.ObjectDict({})
    # # 从配置文件中读取 phantomjs_proxy 的值
    # if 'phantomjs_proxy' in config:
    #     ctx.obj['phantomjs_proxy'] = config['phantomjs_proxy']
...
def cli(ctx, **kwargs):
    ...
    if kwargs.get("config",{"config":{"phantomjs_proxy":"localhost:22222"}}).get('phantomjs_proxy',"localhost:22222"):
        pass
...
def fetcher(ctx, xmlrpc, no_xmlrpc, xmlrpc_host, xmlrpc_port, poolsize, proxy, user_agent,
    ...
    if not async_mode and ("8005" not in str(fetcher.phantomjs_proxy)):
        fetcher.phantomjs_proxy = 'http://192.168.93.207:8009/api/pyspider'
    if async_mode:
        proxy_list_js = ['http://192.168.93.191:8005/api/pyspider','http://192.168.93.207:8005/api/pyspider']
        fetcher.phantomjs_proxy = random.choice(proxy_list_js)
...
@click.option('--process-time-limit', default=20, help='script process time limit')
@click.pass_context
def processor(ctx, processor_cls, process_time_limit, enable_stdout_capture=True, get_object=False):
    ...
    try:
        enable_stdout_capture = g.config.get("enable_stdout_capture",True)
        process_time_limit = g.config.get("process_time_limit",my_process_time_limit)
    except:
        pass
...
@click.option('--processor-num', default=8, help='instance num of processor')
