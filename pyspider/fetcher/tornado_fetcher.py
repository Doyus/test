...
class Fetcher(object):
    ...
    def __init__(self, inqueue, outqueue, poolsize=150, proxy=None, async_mode=True):
    ...
    def http_fetch(self, url, task):
        ...
        while True:
            ...
            try:
                decompress_response = task_fetch['save'].get('decompress_res')
                if decompress_response=='false':
                    fetch['decompress_response'] = False
                # fetch['use_gzip'] = True
                # print("\naaaaaaaaaaa",self.async_mode,  fetch)
    ...
    def puppeteer_fetch(self, url, task):
        ...
        print("self.puppeteer_proxy", self.puppeteer_proxy)
    ...
    def run(self):
        ...
        def queue_loop():
            if not self.outqueue or not self.inqueue:
                logger.warning("outqueue 或 inqueue 未设置。")
                return
            while not self._quit:
                try:
                    if self.outqueue.full():
                        # logger.info("outqueue 已满，退出循环。")
                        break
                    if self.http_client.free_size() <= 0:
                        # logger.info("没有空闲的 HTTP 客户端插槽，退出循环。")
                        break
                    task = self.inqueue.get_nowait()
                    # FIXME: decode unicode_obj 应该在从数据库选择数据后使用，这里为了性能考虑
                    task = utils.decode_unicode_obj(task)
                    # logger.debug(f"解码后的任务: {task}")
                    self.fetch(task)
                except queue.Empty:
                    break
                except KeyboardInterrupt:
                    logger.info("收到键盘中断信号，退出循环。")
                    break
                except Exception as e:
                    logger.exception("queue_loop 中发生异常: %s", e)
                    break
        ...
        try:
            logger.info("启动 IOLoop...")
            self.ioloop.start()
            logger.info("IOLoop 已停止。")
        except KeyboardInterrupt:
            logger.info("收到键盘中断信号，停止 IOLoop。")
            pass
        logger.info("抓取器退出...")
